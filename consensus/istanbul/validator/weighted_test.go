...
func TestNewWeightedCouncil(t *testing.T) {
	...
	// Create ValidatorSet
	valSet := NewWeightedCouncil(ExtractValidators(b), nil, rewardAddrs, votingPowers, nil, istanbul.WeightedRandom, 21, 0, 0, nil, nil)
	if valSet == nil {
...
func TestNormalWeightedCouncil(t *testing.T) {
	...
	// Temporary set IstanbulCompatibleBlock to 0 for testing
	...
	valSet := NewWeightedCouncil([]common.Address{addr1, addr2}, nil, []common.Address{rewardAddr1, rewardAddr2}, []uint64{votingPower1, votingPower2}, []uint64{weight1, weight2}, istanbul.WeightedRandom, 21, 0, 0, nil, nil)
	if valSet == nil {
...
func TestEmptyWeightedCouncil(t *testing.T) {
	valSet := NewWeightedCouncil(ExtractValidators([]byte{}), nil, nil, nil, nil, istanbul.WeightedRandom, 0, 0, 0, &blockchain.BlockChain{}, nil)
	if valSet == nil {
...
func TestNewWeightedCouncil_InvalidPolicy(t *testing.T) {
	...
	valSet := NewWeightedCouncil(ExtractValidators([]byte{}), nil, nil, nil, nil, istanbul.Sticky, 0, 0, 0, &blockchain.BlockChain{}, nil)
	assert.Equal(t, (*weightedCouncil)(nil), valSet)

	valSet = NewWeightedCouncil(ExtractValidators([]byte{}), nil, nil, nil, nil, istanbul.RoundRobin, 0, 0, 0, &blockchain.BlockChain{}, nil)
	assert.Equal(t, (*weightedCouncil)(nil), valSet)
}
...
func TestNewWeightedCouncil_IncompleteParams(t *testing.T) {
	...
	valSet := NewWeightedCouncil(ExtractValidators([]byte{}), nil, rewardAddrs, votingPowers, weights, istanbul.WeightedRandom, 0, 0, 0, &blockchain.BlockChain{}, nil)
	assert.Equal(t, (*weightedCouncil)(nil), valSet)

	// Incomplete rewardAddrs
	incompleteRewardAddrs := make([]common.Address, 1)
	valSet = NewWeightedCouncil(ExtractValidators(b), nil, incompleteRewardAddrs, nil, nil, istanbul.WeightedRandom, 0, 0, 0, &blockchain.BlockChain{}, nil)
	assert.Equal(t, (*weightedCouncil)(nil), valSet)

	// Incomplete rewardAddrs
	incompleteVotingPowers := make([]uint64, 1)
	valSet = NewWeightedCouncil(ExtractValidators(b), nil, nil, incompleteVotingPowers, nil, istanbul.WeightedRandom, 0, 0, 0, &blockchain.BlockChain{}, nil)
	assert.Equal(t, (*weightedCouncil)(nil), valSet)

	// Incomplete rewardAddrs
	incompleteWeights := make([]uint64, 1)
	valSet = NewWeightedCouncil(ExtractValidators(b), nil, nil, nil, incompleteWeights, istanbul.WeightedRandom, 0, 0, 0, &blockchain.BlockChain{}, nil)
	assert.Equal(t, (*weightedCouncil)(nil), valSet)
}
