...
func (api *APIExtension) makeRPCBlockOutput(b *types.Block,
	...
	r, err := klaytnApi.RpcOutputBlock(b, td, false, false, api.chain.Config().IsEthTxTypeForkEnabled(b.Header().Number), api.chain.Config().IsRandaoForkEnabled(b.Header().Number))
	if err != nil {
